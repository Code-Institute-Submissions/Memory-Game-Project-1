...
function showTutorial() { 
    if (hasSecondSquareBeenClicked === false) {                                                     // Detects whether or not the user has selected a second square
        ...
        $(".restart-btn").removeClass("hidden");                                                     // Show the restart button
    };
};
...
$(document).ready(function() {
        $(".game-card").click(function onCardClicked(e) {                                            // When a gamecard is clicked execute the function onCardClicked with parameter "e"
            ...
            if (!clickedCard) {                                                                     // if clickedCard = null
                ...
                if (clickedCard.getAttribute('data-color') !== target.getAttribute('data-color')) { // check to see if the two cards selected are equal                                           // log to the console that the cards are not equal
